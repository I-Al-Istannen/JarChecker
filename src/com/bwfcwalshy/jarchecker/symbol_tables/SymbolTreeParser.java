...
public class SymbolTreeParser {
	 ...
	 * @param source The source to parse
	 * @param sourceFile The source file.
	 * @param extraImports The extra imports to use
	 */
	...
	public void parse() {
		...
		for (String string : source.split(System.lineSeparator())) {
			...
			if(string.trim().startsWith("//") || string.startsWith("import") || string.startsWith("package") || string.matches("[\\s]*\\*.*")
			...
			// Dependencies are only registered if they are imported. Fully qualified names are !ignored! if they are not known. Reason: Method invocations
			...
			for(String word : string.split("([\\s]+)|(\\()|(\\))|(,)")) {
				...
				if(importMap.containsKey(word)) {
					...
					System.out.println("Imported class found: " + word + " <" + varName + "> (" + importMap.get(word) + ") " + string);
	...
	private void addJavaLangImports() throws IOException {
		...
		addImportsFromJar(new File(rtJarPath), entry -> {
			return entry.getName().contains("java/lang") && entry.getName().split("/").length == 3 && !entry.getName().contains("$");
		});
	}

	/**
	 * @param file The file to read from
	 * @param filter The filter the files must match
	 */
	...
	/**
	 * Just for testing. We need to somehow give him ALL the bukkit/spigot imports, so it can correctly resolve those. Maybe in an external file? 
	 * 
